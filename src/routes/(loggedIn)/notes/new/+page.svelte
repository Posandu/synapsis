<script lang="ts">
	...
	const { form, errors, submit, enhance, message } = superForm(data.form, {
		dataType: 'json',
		onSubmit: () => {
			loading = true;
		},
...
<form method="POST" use:enhance>
	<div class="mb-4 flex w-full gap-4 align-baseline">
		...
		<div class="flex-1">
			<input
				type="text"
				class="mb-3 text-3xl font-bold text-black"
				placeholder="New Note"
				bind:value={$form.title}
			/>

			{#if $errors.title}
				<Typography variant="subtitle" class="text-error">
					{$errors.title}
				</Typography>
			{/if}

			<Typography variant="subtitle" class="mb-4 max-w-xl">
				Drop your notes here - type away or snap a pic! For now, we're text-only (math is still
				finding its feet).
			</Typography>
		</div>
	</div>
	...
	<div class="flex gap-8">
		...
		<div class="w-60">
			<Typography variant="h5" class="mb-3">Publish</Typography>
			<Typography variant="subtitle" class="mb-4 max-w-xl">
			...
			<Button variant="primary" {loading} disabled={loading} onclick={() => submit()}>
				Publish Note
			</Button>
